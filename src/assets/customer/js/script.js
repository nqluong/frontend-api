	...
	function headerStyle() {
		if ($('.main-header').length) {
			...
			if (windowpos > 100) {
				...
				// Copy menu từ main-menu vào sticky-header, nếu chưa có
				if ($('.sticky-header .navigation').children().length === 0) {
					$('.main-header .navigation').clone().appendTo('.sticky-header .main-menu');
					
					// Thay thế thẻ <a> thành Angular RouterLink
					$('.sticky-header .navigation a').each(function () {
						var href = $(this).attr('href');
						if (href && !href.startsWith('http')) { // Chỉ thay thế link nội bộ
							$(this).replaceWith('<a [routerLink]="[\'' + href + '\']">' + $(this).text() + '</a>');
						}
					});
				}
			} else {
